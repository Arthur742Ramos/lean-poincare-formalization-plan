 ...
 `SatisfiesIntrinsicEquationAt` using the chosen smooth Levi-Civita witness. It
  ...
  endomorphism, again both statically and slice by slice for connection
     ...
     mathlib's time-dependent Picard-Lindelöf theorem as Banach-model local flows
      ...
      projection lemmas from product local-flow solutions to the base and tangent
       ...
       Lipschitz estimate for the full variational vector field, and
        ...
        the variational package internally. The same operator-domain shape is now
        also named as `CoordinatePullbackMetricOperatorDerivativeWithinOn` and
   ...
   `Diffeomorph3GaugeFlowOn` raw-flow API also has arbitrary-vector-field
    ...
    endpoints through the raw gauge-flow boundary. The ambient local-gluing
    ...
    chosen-time-set repackaging, and matching `_with_Icc_subset_...` variants
    also return the proof that the closed symmetric interval lies inside the
    ambient solution time set. DeTurck local solution records now have exact
...
ball/cylinder radii inside such open neighborhoods, positive exponent
...
finite-product and finite-product-difference closure for
existential normed-comm-ring-valued `C^{0,α}` controls plus an explicit bounded
...
products on both sides under the same determinant lower bound and explicit
bounded estimates for both inverse-vector product orders, explicit bounded
inverse-bilinear contraction estimates, whole finite
...
closed-cylinder spatial-Lipschitz Gram-entry forms for the inverse-Gram,
inverse-Gram vector/vector-inverse product, inverse-Gram bilinear,
inverse-principal, inverse-Gram Christoffel, and schematic RHS bridges,
