...
existential normed-comm-ring-valued `C^{0,α}` controls, product-valued pairing
closure, integer-scalar closure,
...
and bounded `C^{0,α}` composition under range or explicit closed-sup-ball bounds. This is
...
as well as entrywise closure for matrix products, matrix-vector products, and
inverse-matrix-vector products under the same determinant lower bound,
