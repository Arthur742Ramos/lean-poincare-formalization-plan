...
/-- Extract an explicit closed real interval from a relative neighborhood. -/
theorem exists_Icc_inter_subset_of_mem_nhdsWithin
    {s P : Set ℝ} {t : ℝ} (hP : P ∈ 𝓝[s] t) :
    ∃ ε : ℝ, 0 < ε ∧ Icc (t - ε) (t + ε) ∩ s ⊆ P := by
  rcases exists_Ioo_inter_subset_of_mem_nhdsWithin hP with
    ⟨ε, hεpos, hεsub⟩
...
/-- A finite open cover of a compact set admits compact closed cores subordinate
to the original patches and still covering the compact set. -/
theorem exists_compact_closure_core_cover_of_iUnion
    [T2Space X] [LocallyCompactSpace X]
    {ι : Type*} [Finite ι] {s : Set X} {U : ι → Set X}
    (hs : IsCompact s) (hUopen : ∀ i, IsOpen (U i))
    (hcover : s ⊆ ⋃ i, U i) :
    ∃ K : ι → Set X,
      s ⊆ ⋃ i, K i ∧ (∀ i, IsCompact (K i)) ∧ (∀ i, K i ⊆ U i) := by
  ...
  rcases exists_subset_iUnion_closure_subset_t2space
    ...
    ⟨V, hVcover, _hVopen, hVclosure, hVcompact⟩
  refine ⟨fun i ↦ closure (V i), ?_, hVcompact, hVclosure⟩
  intro x hx
...
/-- Closed-interval form of
`timeDependent_iUnion_cover_eventually_of_finite_subset`: after finite patch
persistence is known in a relative time filter, there is an explicit closed
time interval around the base time on which every time still has the selected
patches as a cover, relative to the ambient time set. -/
theorem timeDependent_iUnion_cover_exists_Icc_of_finite_subset
    {ι : Type*} [Finite ι] {s : Set ℝ} {t : ℝ}
    {U : ℝ → ι → Set X}
    (hcover : Set.univ ⊆ ⋃ i, U t i)
    (hwithin : ∀ i, ∀ᶠ τ in 𝓝[s] t, U t i ⊆ U τ i) :
    ∃ ε : ℝ, 0 < ε ∧
      ∀ ⦃τ : ℝ⦄, τ ∈ Icc (t - ε) (t + ε) → τ ∈ s →
        Set.univ ⊆ ⋃ i, U τ i := by
  rcases exists_Icc_inter_subset_of_mem_nhdsWithin
      (timeDependent_iUnion_cover_eventually_of_finite_subset hcover hwithin) with
    ⟨ε, hεpos, hεsub⟩
  refine ⟨ε, hεpos, ?_⟩
  intro τ hτIcc hτs
...
patches must cover throughout one closed Picard interval. -/
theorem timeDependent_iUnion₂_cover_exists_Icc_of_finite_subset
    {ι : Type*} [Finite ι] {s : Set ℝ} {t : ℝ}
    {U V : ℝ → ι → Set X}
    (hUcover : Set.univ ⊆ ⋃ i, U t i)
    (hVcover : Set.univ ⊆ ⋃ i, V t i)
    (hUwithin : ∀ i, ∀ᶠ τ in 𝓝[s] t, U t i ⊆ U τ i)
    (hVwithin : ∀ i, ∀ᶠ τ in 𝓝[s] t, V t i ⊆ V τ i) :
    ∃ ε : ℝ, 0 < ε ∧
      ∀ ⦃τ : ℝ⦄, τ ∈ Icc (t - ε) (t + ε) → τ ∈ s →
        (Set.univ ⊆ ⋃ i, U τ i) ∧ (Set.univ ⊆ ⋃ i, V τ i) := by
  rcases timeDependent_iUnion_cover_exists_Icc_of_finite_subset
      (U := U) hUcover hUwithin with
    ⟨εU, hεUpos, hUsub⟩
  rcases timeDependent_iUnion_cover_exists_Icc_of_finite_subset
      (U := V) hVcover hVwithin with
    ⟨εV, hεVpos, hVsub⟩
  refine ⟨min εU εV, lt_min hεUpos hεVpos, ?_⟩
  intro τ hτIcc hτs
  have hτU : τ ∈ Icc (t - εU) (t + εU) := by
    constructor <;> linarith [hτIcc.1, hτIcc.2, min_le_left εU εV]
...
/-- Build a raw `C^3` gauge-flow witness from compatible local readouts on
time-dependent open covers when each local time-slice is supplied as
`LocalGluingData` and pointwise source persistence is derived from fixed open
target patches along the local forward readouts. -/
noncomputable def of_timeDependent_iUnion_compatibleGluedSlices_openPreimage_localGluingData_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
  ...
  of_timeDependent_iUnion_compatibleGluedSlices_openPreimage_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    (I := I) (M := M) (X := X) (Y := Y)
    (tmin := tmin) (tmax := tmax) (t₀ := t₀)
    defaultF defaultG Fₗ Gₗ U V W hUcover hVcover
...
/-- Proof-level raw `C^3` gauge-flow existence from compatible local readouts
with named `LocalGluingData` patches and open-preimage source persistence. -/
theorem nonempty_of_timeDependent_iUnion_compatibleGluedSlices_openPreimage_localGluingData_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
    ...
    (hYLocal : ∀ t ∈ Ioo tmin tmax, ∀ᶠ τ in 𝓝[Ioo tmin tmax] t,
      ∀ i, ∀ x : M, x ∈ U τ i → Y τ (Fₗ i τ x) = X τ (Fₗ i τ x)) :
    Nonempty (Diffeomorph3GaugeFlowOn (I := I) (M := M) X (Ioo tmin tmax) t₀) :=
  ⟨of_timeDependent_iUnion_compatibleGluedSlices_openPreimage_localGluingData_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    (I := I) (M := M) (X := X) (Y := Y)
    (tmin := tmin) (tmax := tmax) (t₀ := t₀)
    defaultF defaultG Fₗ Gₗ U V W hUcover hVcover hlocal hFcompat hGcompat
    hUpreimage hWopen hanchoredLocal hcontLocal hderivLocal hYLocal⟩

/-- Build a raw `C^3` gauge-flow witness from compatible local readouts whose
time-dependent covers are only known on the closed Picard interval.

The wrapper extends the local system to all time by adding an `Option.none`
identity patch outside `Icc tmin tmax` and an empty `Option.none` patch inside
the interval.  This supplies the all-time covers required by
`SmoothSelfDiffeomorph3Family.ofInverse` while keeping the analytic hypotheses
interval-local. -/
noncomputable def of_Icc_timeDependent_iUnion_compatibleGluedSlices_pointwiseSource_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
  ...
  · intro t ht
    filter_upwards [hYLocal t ht, self_mem_nhdsWithin] with τ hYτ hτIoo i x hx
    have hτIcc : τ ∈ Icc tmin tmax := Ioo_subset_Icc_self hτIoo
    have hτT : τ ∈ T := by simpa [T] using hτIcc
    cases i with
...
/-- Proof-level raw `C^3` gauge-flow existence from compatible local readouts
whose time-dependent covers are only known on the closed Picard interval. -/
theorem nonempty_of_Icc_timeDependent_iUnion_compatibleGluedSlices_pointwiseSource_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
...
This is the interval-cover analogue of the all-time local-gluing-data wrappers:
the source and target covers are only required on `Icc tmin tmax`, while an
identity fallback patch supplies the global smooth-family data outside that
interval. -/
noncomputable def of_Icc_timeDependent_iUnion_localGluingData_pointwiseSource_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
...
/-- Proof-level raw `C^3` gauge-flow existence from interval-local compatible
readouts supplied as `LocalGluingData`. -/
theorem nonempty_of_Icc_timeDependent_iUnion_localGluingData_pointwiseSource_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
...
noncomputable def of_Icc_timeDependent_iUnion_openPreimage_localGluingData_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
...
/-- Proof-level raw `C^3` gauge-flow existence from interval-local compatible
`LocalGluingData` readouts and fixed open target-preimage patches. -/
theorem nonempty_of_Icc_timeDependent_iUnion_openPreimage_localGluingData_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
...
noncomputable def of_Icc_subset_timeSet_timeDependent_iUnion_openPreimage_localGluingData_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {timeSet : Set ℝ} {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
    (U V : ℝ → ι → Set M) (W : ι → Set M)
    (hIcc_subset : Icc tmin tmax ⊆ timeSet)
    (hUcover : ∀ t ∈ Icc tmin tmax, Set.univ ⊆ ⋃ i, U t i)
    (hVcover : ∀ t ∈ Icc tmin tmax, Set.univ ⊆ ⋃ i, V t i)
    (hlocal : ∀ t ∈ Icc tmin tmax, ∀ i,
      LocalGluingData (I := I) (M := M) 3 (Fₗ i t) (Gₗ i t) (U t i) (V t i))
  ...
  of_Icc_timeDependent_iUnion_openPreimage_localGluingData_of_local_hasDerivWithinAt_Icc_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    ...
    (tmin := tmin) (tmax := tmax) (t₀ := t₀)
    defaultF defaultG Fₗ Gₗ U V W hUcover hVcover hlocal hFcompat
    hGcompat hUpreimage hWopen ht₀ hanchoredLocal
    (fun i t ht x hx ↦ (hcontLocal i t ht x hx).mono hIcc_subset)
    (fun i t ht x hx ↦ (hderivLocal i t ht x hx).mono hIcc_subset)
    hYLocal

/-- Proof-level raw `C^3` gauge-flow existence from interval-local compatible
`LocalGluingData` readouts with local regularity stated on an ambient time set
containing the closed Picard interval. -/
theorem nonempty_of_Icc_subset_timeSet_timeDependent_iUnion_openPreimage_localGluingData_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {timeSet : Set ℝ} {tmin tmax t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
    ...
    (hYLocal : ∀ t ∈ Ioo tmin tmax, ∀ᶠ τ in 𝓝[Ioo tmin tmax] t,
      ∀ i, ∀ x : M, x ∈ U τ i → Y τ (Fₗ i τ x) = X τ (Fₗ i τ x)) :
    Nonempty (Diffeomorph3GaugeFlowOn (I := I) (M := M) X (Ioo tmin tmax) t₀) :=
  ⟨of_Icc_subset_timeSet_timeDependent_iUnion_openPreimage_localGluingData_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    (I := I) (M := M) (X := X) (Y := Y)
    (timeSet := timeSet) (tmin := tmin) (tmax := tmax) (t₀ := t₀)
    defaultF defaultG Fₗ Gₗ U V W hIcc_subset hUcover hVcover hlocal
    hFcompat hGcompat hUpreimage hWopen ht₀ hanchoredLocal hcontLocal
    hderivLocal hYLocal⟩

/-- Finite-family raw `C^3` gauge-flow existence from compact source and target
cores.  The moving source and target covers on one closed Picard interval are
derived from fixed target-preimage descriptions and space-time continuity,
rather than assumed as set-level temporal persistence of open patches. -/
theorem exists_Ioo_gaugeFlow_of_finite_timeSet_compactCore_openPreimage_localGluingData_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    {ι : Type*} [Finite ι]
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {timeSet : Set ℝ} {t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
    (U V : ℝ → ι → Set M) (W Z : ι → Set M) (KU KV : ι → Set M)
    (htimeSet : timeSet ∈ 𝓝 t₀)
    (hlocal : ∀ t : ℝ, ∀ i,
      LocalGluingData (I := I) (M := M) 3 (Fₗ i t) (Gₗ i t) (U t i) (V t i))
    (hFcompat : ∀ t : ℝ, ∀ i j, EqOn (Fₗ i t) (Fₗ j t) (U t i ∩ U t j))
    (hGcompat : ∀ t : ℝ, ∀ i j, EqOn (Gₗ i t) (Gₗ j t) (V t i ∩ V t j))
    (hUpreimage : ∀ τ : ℝ, ∀ i, ∀ x : M, x ∈ U τ i ↔ Fₗ i τ x ∈ W i)
    (hVpreimage : ∀ τ : ℝ, ∀ i, ∀ x : M, x ∈ V τ i ↔ Gₗ i τ x ∈ Z i)
    (hKUcompact : ∀ i, IsCompact (KU i))
    (hKVcompact : ∀ i, IsCompact (KV i))
    (hWopen : ∀ i, IsOpen (W i))
    (hZopen : ∀ i, IsOpen (Z i))
    (hKUcover : Set.univ ⊆ ⋃ i, KU i)
    (hKVcover : Set.univ ⊆ ⋃ i, KV i)
    (hKUbase : ∀ i, KU i ⊆ U t₀ i)
    (hKVbase : ∀ i, KV i ⊆ V t₀ i)
    (hUspaceCont : ∀ i, ∀ x ∈ KU i,
      ContinuousWithinAt (fun p : ℝ × M ↦ Fₗ i p.1 p.2)
        (timeSet ×ˢ Set.univ) (t₀, x))
    (hVspaceCont : ∀ i, ∀ x ∈ KV i,
      ContinuousWithinAt (fun p : ℝ × M ↦ Gₗ i p.1 p.2)
        (timeSet ×ˢ Set.univ) (t₀, x))
    (hanchoredLocal : ∀ i, ∀ x ∈ U t₀ i, Fₗ i t₀ x = x)
    ...
    ∃ ε : ℝ, 0 < ε ∧
      Nonempty (Diffeomorph3GaugeFlowOn (I := I) (M := M) X
        (Ioo (t₀ - ε) (t₀ + ε)) t₀) := by
  rcases timeDependent_iUnion₂_cover_exists_Icc_subset_of_finite_compact_openPreimage_continuousWithinAt_prod
      (X := M) (Y := M) (timeSet := timeSet) (t₀ := t₀)
      (F₁ := Fₗ) (F₂ := Gₗ) (U₁ := U) (U₂ := V)
      (W₁ := W) (W₂ := Z) (K₁ := KU) (K₂ := KV)
      htimeSet hUpreimage hVpreimage hKUcompact hKVcompact hWopen hZopen
      hKUcover hKVcover hKUbase hKVbase hUspaceCont hVspaceCont with
    ⟨ε, hεpos, hIcc_subset, hcover_Icc⟩
  refine ⟨ε, hεpos, ?_⟩
  have ht₀Icc : t₀ ∈ Icc (t₀ - ε) (t₀ + ε) := by
    constructor <;> linarith
  exact
    nonempty_of_Icc_subset_timeSet_timeDependent_iUnion_openPreimage_localGluingData_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
      (I := I) (M := M) (X := X) (Y := Y)
      (timeSet := timeSet) (tmin := t₀ - ε) (tmax := t₀ + ε) (t₀ := t₀)
      defaultF defaultG Fₗ Gₗ U V W hIcc_subset
      (fun t ht ↦ (hcover_Icc ht).1)
      (fun t ht ↦ (hcover_Icc ht).2)
      (fun t _ht i ↦ hlocal t i)
      (fun t _ht i j ↦ hFcompat t i j)
      (fun t _ht i j ↦ hGcompat t i j)
      hUpreimage hWopen ht₀Icc hanchoredLocal
      (fun i t ht x hx ↦ hcontLocal i t (hIcc_subset ht) x hx)
      (fun i t ht x hx ↦ hderivLocal i t (hIcc_subset ht) x hx)
      (fun t ht ↦ by
        have hIoo_subset_timeSet : Ioo (t₀ - ε) (t₀ + ε) ⊆ timeSet := fun τ hτ ↦
          hIcc_subset (Ioo_subset_Icc_self hτ)
        have htTime : t ∈ timeSet := hIoo_subset_timeSet ht
        filter_upwards
          [(hYLocal t htTime).filter_mono
            (nhdsWithin_mono t hIoo_subset_timeSet)] with τ hYτ
        intro i x hx
        exact hYτ i x hx)

/-- Finite-family raw `C^3` gauge-flow existence on a compact manifold from
base-time source cover data.  Compact closed cores subordinate to the base-time
source and target patches are selected automatically by the shrinking lemma;
their persistence is then derived from fixed target-preimage descriptions and
space-time continuity. -/
theorem exists_Ioo_gaugeFlow_of_finite_compact_timeSet_iUnion_openPreimage_localGluingData_of_spaceTime_continuous_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    [CompactSpace M] {ι : Type*} [Finite ι]
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {timeSet : Set ℝ} {t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
    (U V : ℝ → ι → Set M) (W Z : ι → Set M)
    (htimeSet : timeSet ∈ 𝓝 t₀)
    (hlocal : ∀ t : ℝ, ∀ i,
      LocalGluingData (I := I) (M := M) 3 (Fₗ i t) (Gₗ i t) (U t i) (V t i))
    (hFcompat : ∀ t : ℝ, ∀ i j, EqOn (Fₗ i t) (Fₗ j t) (U t i ∩ U t j))
    ...
    (hZopen : ∀ i, IsOpen (Z i))
    (hUcover : Set.univ ⊆ ⋃ i, U t₀ i)
    (hUspaceCont : ∀ i, ∀ x ∈ U t₀ i,
      ContinuousWithinAt (fun p : ℝ × M ↦ Fₗ i p.1 p.2)
        (timeSet ×ˢ Set.univ) (t₀, x))
    (hVspaceCont : ∀ i, ∀ x ∈ V t₀ i,
      ContinuousWithinAt (fun p : ℝ × M ↦ Gₗ i p.1 p.2)
        (timeSet ×ˢ Set.univ) (t₀, x))
    (hanchoredLocal : ∀ i, ∀ x ∈ U t₀ i, Fₗ i t₀ x = x)
    (hcontLocal : ∀ i, ∀ t ∈ timeSet, ∀ x : M, x ∈ U t i →
  ...
  rcases exists_compact_closure_core_cover_of_iUnion
      (s := (Set.univ : Set M)) (U := U t₀) isCompact_univ
      (fun i ↦ (hlocal t₀ i).source_open) hUcover with
    ⟨KU, hKUcover, hKUcompact, hKUbase⟩
  rcases exists_compact_closure_core_cover_of_iUnion
      (s := (Set.univ : Set M)) (U := V t₀) isCompact_univ
      (fun i ↦ (hlocal t₀ i).target_open) hVcover with
    ⟨KV, hKVcover, hKVcompact, hKVbase⟩
  exact
    exists_Ioo_gaugeFlow_of_finite_timeSet_compactCore_openPreimage_localGluingData_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
      (I := I) (M := M) (X := X) (Y := Y)
      (timeSet := timeSet) (t₀ := t₀)
      defaultF defaultG Fₗ Gₗ U V W Z KU KV htimeSet hlocal hFcompat
      hGcompat hUpreimage hVpreimage hKUcompact hKVcompact hWopen hZopen
      hKUcover hKVcover hKUbase hKVbase
      (fun i x hx ↦ hUspaceCont i x (hKUbase i hx))
      (fun i x hx ↦ hVspaceCont i x (hKVbase i hx))
      hanchoredLocal hcontLocal hderivLocal hYLocal

/-- Compact raw `C^3` gauge-flow existence from arbitrary indexed local data
using finite source-subcover selection and compact-core persistence.  Compared
with the older compact raw constructor, source and target cover persistence are
derived from fixed source/target preimage descriptions plus space-time
continuity at the base slice. -/
theorem exists_Ioo_gaugeFlow_of_compact_timeSet_iUnion_openPreimage_localGluingData_of_spaceTime_continuous_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
  ...
  rcases isCompact_univ.elim_finite_subcover (U t₀)
      (fun i ↦ (hlocal t₀ i).source_open) hUcover with
    ⟨s, hscover⟩
  have hUcover_sub : Set.univ ⊆ ⋃ i : {i // i ∈ s}, U t₀ i := by
    intro x hx
    rcases Set.mem_iUnion₂.mp (hscover hx) with ⟨i, hi, hxi⟩
    exact Set.mem_iUnion.mpr ⟨⟨i, hi⟩, hxi⟩
  exact
    exists_Ioo_gaugeFlow_of_finite_compact_timeSet_iUnion_openPreimage_localGluingData_of_spaceTime_continuous_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
      (I := I) (M := M) (X := X) (Y := Y)
      (timeSet := timeSet) (t₀ := t₀)
      defaultF defaultG
      (fun i : {i // i ∈ s} ↦ Fₗ i)
      ...
      (fun t i j ↦ hFcompat t i j)
      (fun t i j ↦ hGcompat t i j)
      (fun τ i x ↦ hUpreimage τ i x)
      (fun τ i x ↦ hVpreimage τ i x)
      (fun i ↦ hWopen i)
      (fun i ↦ hZopen i)
      hUcover_sub
      (fun i x hx ↦ hUspaceCont i x hx)
      (fun i x hx ↦ hVspaceCont i x hx)
      (fun i x hx ↦ hanchoredLocal i x hx)
      (fun i t ht x hx ↦ hcontLocal i t ht x hx)
      (fun i t ht x hx ↦ hderivLocal i t ht x hx)
      (fun t ht ↦ by
        filter_upwards [hYLocal t ht] with τ hYτ i x hx
        exact hYτ i x hx)

/-- Compact raw `C^3` gauge-flow existence from local gluing patches that are
first restricted to paired fixed-preimage patches.  This is the handoff from
ordinary local inverse-function output to the compact constructor whose moving
source and target patches are exactly `Fₗ i τ ⁻¹' W i` and `Gₗ i τ ⁻¹' Z i`. -/
theorem exists_Ioo_gaugeFlow_of_compact_timeSet_iUnion_restrictedPairPreimage_localGluingData_of_spaceTime_continuous_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
    [CompactSpace M] {ι : Type*}
    {X Y : CovariantDerivative.TimeDependentVectorField (I := I) (M := M)}
    {timeSet : Set ℝ} {t₀ : ℝ}
    (defaultF defaultG : ℝ → M → M) (Fₗ Gₗ : ι → ℝ → M → M)
    ...
    (hWsub : ∀ t i, W i ⊆ V t i)
    (hZsub : ∀ t i, Z i ⊆ U t i)
    (hFpreOpen : ∀ t i, IsOpen ((Fₗ i t) ⁻¹' W i))
    (hGpreOpen : ∀ t i, IsOpen ((Gₗ i t) ⁻¹' Z i))
    (hFpreSubZ : ∀ t i, (Fₗ i t) ⁻¹' W i ⊆ Z i)
    (hGpreSubW : ∀ t i, (Gₗ i t) ⁻¹' Z i ⊆ W i)
    (hUcover : Set.univ ⊆ ⋃ i, (Fₗ i t₀) ⁻¹' W i)
    (hUspaceCont : ∀ i, ∀ x ∈ U t₀ i,
      ContinuousWithinAt (fun p : ℝ × M ↦ Fₗ i p.1 p.2)
        (timeSet ×ˢ Set.univ) (t₀, x))
    (hVspaceCont : ∀ i, ∀ x ∈ V t₀ i,
      ContinuousWithinAt (fun p : ℝ × M ↦ Gₗ i p.1 p.2)
        (timeSet ×ˢ Set.univ) (t₀, x))
    (hanchoredLocal : ∀ i, ∀ x ∈ (Fₗ i t₀) ⁻¹' W i, Fₗ i t₀ x = x)
    (hcontLocal : ∀ i, ∀ t ∈ timeSet, ∀ x : M, x ∈ U t i →
      ContinuousWithinAt (fun τ : ℝ ↦ Fₗ i τ x) timeSet t)
    (hderivLocal : ∀ i, ∀ t ∈ timeSet, ∀ x : M, x ∈ U t i →
      HasDerivWithinAt
    ...
    ∃ ε : ℝ, 0 < ε ∧
      Nonempty (Diffeomorph3GaugeFlowOn (I := I) (M := M) X
        (Ioo (t₀ - ε) (t₀ + ε)) t₀) := by
  rcases LocalGluingData.timeDependent_restrict_preimages
      (I := I) (M := M) Fₗ Gₗ U V W Z hlocal hFcompat hGcompat
      hWopen hZopen hWsub hZsub hFpreOpen hGpreOpen hFpreSubZ hGpreSubW with
    ⟨hlocal_pre, hFcompat_pre, hGcompat_pre⟩
  exact
    exists_Ioo_gaugeFlow_of_compact_timeSet_iUnion_openPreimage_localGluingData_of_spaceTime_continuous_of_local_hasDerivWithinAt_timeSet_extChartAt_eval_self_of_vectorField_eq_nhdsWithin
      (I := I) (M := M) (X := X) (Y := Y)
      (timeSet := timeSet) (t₀ := t₀)
      defaultF defaultG Fₗ Gₗ
      (fun τ i ↦ (Fₗ i τ) ⁻¹' W i)
      (fun τ i ↦ (Gₗ i τ) ⁻¹' Z i)
      W Z htimeSet hlocal_pre hFcompat_pre hGcompat_pre
      (fun _τ _i _x ↦ Iff.rfl)
      (fun _τ _i _x ↦ Iff.rfl)
      hWopen hZopen hUcover
      (fun i x hx ↦ hUspaceCont i x (hZsub t₀ i (hFpreSubZ t₀ i hx)))
      (fun i x hx ↦ hVspaceCont i x (hWsub t₀ i (hGpreSubW t₀ i hx)))
      hanchoredLocal
      (fun i t ht x hx ↦ hcontLocal i t ht x (hZsub t i (hFpreSubZ t i hx)))
      (fun i t ht x hx ↦ hderivLocal i t ht x (hZsub t i (hFpreSubZ t i hx)))
      (fun t ht ↦ by
        filter_upwards [hYLocal t ht] with τ hYτ i x hx
        exact hYτ i x (hZsub τ i (hFpreSubZ τ i hx)))

/-- Compact raw `C^3` gauge-flow existence on a small symmetric interval from
ambient-time local gluing data.

This combines compact finite-subcover selection, source/target cover
...
repackaging.  The base-time target cover and fixed target-preimage cover are
derived from the base-time source cover and the anchoring hypotheses. -/
...
/-- Fixed-IVP raw intrinsic DeTurck gauge-flow existence from compatible local
readouts with named `LocalGluingData` patches whose source persistence is
derived from fixed open target patches along the local forward readouts. -/
noncomputable def ofPicardIccChartDerivative_of_timeDependent_iUnion_compatibleGluedSlices_openPreimage_localGluingData_of_localReadouts_vectorField_eq_nhdsWithin
    {ι : Type*}
    {ivp : InitialValueProblem (E := E) (H := H) (I := I) (M := M)}
    (defaultF defaultG : ∀ _sol : ChosenIntrinsicDeTurckLocalSolution
        (E := E) (H := H) (I := I) (M := M) ivp,
...
/-- Fixed-IVP open-preimage local-gluing-data gauge-flow existence, kept as
proof-level evidence. -/
theorem nonempty_ofPicardIccChartDerivative_of_timeDependent_iUnion_compatibleGluedSlices_openPreimage_localGluingData_of_localReadouts_vectorField_eq_nhdsWithin
    {ι : Type*}
    {ivp : InitialValueProblem (E := E) (H := H) (I := I) (M := M)}
    (defaultF defaultG : ∀ _sol : ChosenIntrinsicDeTurckLocalSolution
        (E := E) (H := H) (I := I) (M := M) ivp,
    ...
    Nonempty (IntrinsicDeTurckGaugeFlowExistence
      (E := E) (H := H) (I := I) (M := M) ivp) :=
  ⟨ofPicardIccChartDerivative_of_timeDependent_iUnion_compatibleGluedSlices_openPreimage_localGluingData_of_localReadouts_vectorField_eq_nhdsWithin
    defaultF defaultG Fₗ Gₗ U V W hUcover hVcover hlocal hFcompat hGcompat
    tmin tmax hUpreimage hWopen hanchoredLocal Y htimeSet hcontLocal
    hderivLocal hYLocal⟩
