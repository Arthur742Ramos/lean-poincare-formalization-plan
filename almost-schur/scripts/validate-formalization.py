...
def main():
    ...
    expected_theorems = [
        "AlmostSchurEntry.almostSchur_from_identities",
        "AlmostSchurEntry.almostSchur_equality_data",
    ]
    ...
    if comparator.get("definition_names") != []:
        raise SystemExit("Comparator definition surface must remain empty")
    ...
    if len(re.findall(r"\btheorem\s+", challenge_text)) != 2:
        raise SystemExit("Challenge theorem count is not exactly two")
    if len(re.findall(r"\bsorry\b", challenge_text)) != 2:
        raise SystemExit("Challenge must contain exactly two intentional holes")
