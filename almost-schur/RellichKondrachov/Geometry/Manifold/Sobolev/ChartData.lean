...
variable {I : ModelWithCorners ℝ E H} [IsManifold I (↑(⊤ : ℕ∞) : WithTop ℕ∞) M]
...
structure FiniteChartData (H : Type*) (M : Type*) [TopologicalSpace H] [TopologicalSpace M]
    [ChartedSpace H M] (I : ModelWithCorners ℝ E H) [IsManifold I (↑(⊤ : ℕ∞) : WithTop ℕ∞) M] where
...
lemma vendorGeometryManifoldSobolevChartData_univ_subset_iUnion_chartAt_source :
    (univ : Set M) ⊆ ⋃ x : M, (chartAt H x).source := by
  intro x hx
  exact mem_iUnion_of_mem x (mem_chart_source H x)

omit [T2Space M] [CompactSpace M] in
lemma vendorGeometryManifoldSobolevChartData_isOpen_chartAt_source (x : M) : IsOpen ((chartAt H x).source : Set M) :=
...
theorem exists_finiteChartData_chartAt :
  ...
  have hcover : (univ : Set M) ⊆ ⋃ x : M, (chartAt H x).source :=
    vendorGeometryManifoldSobolevChartData_univ_subset_iUnion_chartAt_source (H := H)
  rcases (isCompact_univ.elim_finite_subcover (fun x : M => (chartAt H x).source)
      (fun x => vendorGeometryManifoldSobolevChartData_isOpen_chartAt_source (H := H) (x := x)) hcover) with ⟨t, ht⟩
  ...
  have ho : ∀ i : ι', IsOpen ((chartAt H (i : M)).source : Set M) := fun i =>
    vendorGeometryManifoldSobolevChartData_isOpen_chartAt_source (H := H) (x := (i : M))
