...
public import RellichKondrachov.Geometry.Manifold.Sobolev.EmbeddingL2
...
lemma vendorGeometryManifoldSobolevRellichKondrachov_isCompactOperator_fintype_sum {X Y : Type*} [TopologicalSpace X] [AddCommMonoid X]
...
theorem isCompactOperator_h1ToL2_of_summands (μ : Measure M) [IsFiniteMeasure μ]
  ...
  simpa [h1ToL2] using!
    (vendorGeometryManifoldSobolevRellichKondrachov_isCompactOperator_fintype_sum
      (X := ↥(h1 (d := d) (I := I) (μ := μ)))
      (Y := (M →₂[μ] ℝ))
      (ι := d.ι)
      (f := fun i x =>
