...
Write
```text
D(p,a,b,c,d) = g(∇p(R(a,b)c) − R(∇p a,b)c
                         − R(a,∇p b)c − R(a,b)∇p c, d).
```
The formal definition uses canonical smooth extensions of the tangent
vectors. For an orthonormal basis b at x, the selected conclusion is
```text
...
instance also explicit). It requires neither compactness nor sigma-compactness.
...
derivative and its raw commutator. The project contains its local dependency
closure; it has no path dependency on the parent project.
