"""Check the exact transitive axiom surface of the selected theorem."""
...
def reported(output: str, name: str) -> set[str]:
    match = re.search(rf"'{re.escape(name)}' depends on axioms: \[([^\]]*)\]",
                      output, re.DOTALL)
    if not match:
...
def main() -> None:
    result = run(f"import TensorHeatSolution\n#print axioms {THEOREM}\n")
    output = result.stdout + result.stderr
    if result.returncode or reported(output, THEOREM) != ALLOWED:
        raise SystemExit("unexpected selected-theorem axiom surface:\n" + output)
    control_name = "TensorHeatForbiddenControl"
    control = run(
        "axiom TensorHeatForbidden : False\n"
        f"theorem {control_name} : True := False.elim TensorHeatForbidden\n"
        f"#print axioms {control_name}\n"
    )
    control_output = control.stdout + control.stderr
    if control.returncode or reported(control_output, control_name) != {"TensorHeatForbidden"}:
