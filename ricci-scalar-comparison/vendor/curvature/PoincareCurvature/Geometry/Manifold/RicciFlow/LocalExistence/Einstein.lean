...
/- Adapted from the recorded same-repository source: this package makes
transported inner-product instances and a real-derivative normalization
explicit for Lean 4.33.0, and removes one redundant tactic. -/
...
theorem isMetricCompatibleTangent_of_inner_eq_const_smul (c : ℝ)
  ...
  have hf : MDifferentiableAt I 𝓘(ℝ, ℝ) (fun y ↦ g.inner y (σ y) (τ y)) x := by
    letI : Bundle.RiemannianBundle TM := ⟨g.toRiemannianMetric⟩
    change MDifferentiableAt I 𝓘(ℝ, ℝ) (fun y ↦ inner ℝ (σ y) (τ y)) x
...
lemma hasDerivAt_homotheticFactor (lam t₀ t : ℝ) :
  ...
  have hd := h.const_sub 1
  change HasDerivAt (fun s : ℝ ↦ 1 - 2 * lam * (s - t₀)) (-(2 * lam)) t
  simpa only [HasDerivAt, HasDerivAtFilter,
