...
variable-radius finite-cover Holder and `C^{0,α}` patching, and finite-sum
closure for explicit Holder, bounded, and `C^{0,α}` controls, and finite-sum
closure for existential Holder and `C^{0,α}` controls, and
