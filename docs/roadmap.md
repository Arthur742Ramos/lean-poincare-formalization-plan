...
to a dedicated continuous-section wrapper. Coordinatewise compact-readout
...
and shared-radius RHS estimates expose that coarsening step directly, as do the
state-space `LipschitzOnWith` coordinate bridges in single-family and
...
handoff, with matching local-frame Gram-coordinate forms.
   ...
   pushforward identities, extracts the scalar inner-product derivative
    ...
    (`Diffeomorph3GaugeFlowOn`) now exposes identity-flow constructors for
     ...
     chosen-Levi-Civita-background case. It also has fixed-IVP and theorem-family
      ...
      projections. The raw `Diffeomorph3GaugeFlowOn` layer also exposes
       ...
       pointwise time-continuity, while the finite-cover local-readout variant derives the uniform
     ...
     time-derivative proofs or scalar inner-product derivative proofs. The raw
      flow layer also has fixed-IVP and theorem-family constructors from both
        ...
        The fixed-IVP and theorem-family intrinsic existence layers now have the
        matching closed-Picard model-field constructors into
        `IntrinsicDeTurckGaugeFlowExistence(.Family)`. The same seam includes direct raw,
        ...
        variants that derive the tangent-coordinate derivative from the model
         ...
         derivative domain is open and contains the Picard graph endpoint, model
          ...
          or eventual membership, now with matching within-filter and ordinary
          eventual-equality transfer companions, plus raw tensor wrappers that
          consume those closed-domain scalar readouts directly, matching closed
          Picard/Banach state domains.
          It also has
     ...
     Picard-Lindelöf theorem as local model flows with initialization on a
      ...
      ordinary open-cylinder versions of those readouts, and `ContinuousOn` over
      the open Picard cylinder,
      named base-flow, tangent-map,
        and vector-slot time-slice continuity bridges for continuous/variational
           ...
           metric-coordinate-field routes (including geometric pushed-vector
           slots and finite-cover/readout-field forms), metric-coordinate-field
      ...
      identifications are only closed-interval within-filter equal, including
      pushed-forward geometric-slot forms, same-set raw gauge-flow moving-base field constructors
      ...
      continuity, chart-coordinate continuity, preferred-chart range eventuality,
      and tangent-trivialization readouts in both within-time-set and
      ordinary-neighborhood forms,
      ...
      component continuity; continuous
       ...
       and componentwise-continuity estimate wrappers, including common-`Ioo`
       overlap forms for backward time slices, as well. The dynamic pullback layer now uses a
      ...
      `B(τ)(A(τ)u)(A(τ)v)` scalar expression; eventual target-trivialization
       ...
       metric-component readouts. The same conversion is available in
        ...
        derivative along the same raw coordinate curve is supplied; its
         ...
         finite-cover field readout can feed this formulation directly; a
         raw-flow component-data constructor and tensor endpoint now package the
         required `Btime = Bfull - spatial` subtraction automatically, including
         a closed-Picard `Ioo` wrapper for raw interval flows. The
    ...
    model-space theorem-family aliases. The smooth Ricci-DeTurck global and
     ...
     readouts. The preferred-cover local-bounds smooth-approximation route now
      ...
      metric-cone shrink, including a proof-level `Nonempty` readout. The
      built-in chart-derived symmetric carrier now has
