...
public import RellichKondrachov.Analysis.FunctionalSpaces.Sobolev.Euclidean.L2Compactness.Smoothing
...
lemma vendorAnalysisFunctionalSpacesSobolevEuclideanL2CompactnessApproximation_instIsProbabilityMeasure_kernelMeasure
...
lemma vendorAnalysisFunctionalSpacesSobolevEuclideanL2CompactnessApproximation_norm_sq_eq_integral_sq (v : (E →₂[(volume : Measure E)] ℝ)) :
...
lemma norm_sq_translateL2_sub_extendByZeroL2_eq_integral_sq (t : E)
  ...
  have hcalc :
    ...
    have h1 :=
      vendorAnalysisFunctionalSpacesSobolevEuclideanL2CompactnessApproximation_norm_sq_eq_integral_sq (E := E) (v := ((translateL2 (μ := (volume : Measure E)) (-t)) F - F))
    have h2 :
...
lemma vendorAnalysisFunctionalSpacesSobolevEuclideanL2CompactnessApproximation_kernelMeasure_le_smul_volume (hψc : Continuous ψ) (hψcs : HasCompactSupport ψ)
    (hψ0 : ∀ x, 0 ≤ ψ x) :
...
lemma smoothFun_sub_extendByZeroFun_sq_le_integral_sq
  ...
  haveI : MeasureTheory.IsProbabilityMeasure μ :=
    vendorAnalysisFunctionalSpacesSobolevEuclideanL2CompactnessApproximation_instIsProbabilityMeasure_kernelMeasure (E := E) (ψ := ψ) hψc hψcs hψ0 hψint
  haveI : MeasureTheory.IsFiniteMeasure μ := by infer_instance
  rcases vendorAnalysisFunctionalSpacesSobolevEuclideanL2CompactnessApproximation_kernelMeasure_le_smul_volume (E := E) (ψ := ψ) hψc hψcs hψ0 with ⟨c, hc_top, hμle⟩
  let f : E → ℝ := extendByZeroFun (E := E) (K := K) u
...
lemma norm_sq_smoothL2_sub_extendByZeroL2_le_integral_norm_sq_translateL2_sub_extendByZeroL2
  ...
  have hnorm_sq :
    ...
    have h1 := vendorAnalysisFunctionalSpacesSobolevEuclideanL2CompactnessApproximation_norm_sq_eq_integral_sq (E := E) (v := (S - F))
    have h2 :
