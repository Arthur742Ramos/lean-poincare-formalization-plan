...
/-- Covariant differentiation along fields respects regular section germs. -/
theorem along_eventuallyEq_of_contMDiffAt
    ...
    cov.along X Z =ᶠ[𝓝 x] cov.along X' Z' := by
...
theorem curvatureAux_eq_of_local_germs
    ...
    cov.curvatureAux X Y Z x = cov.curvatureAux X' Y' Z' x := by
  ...
  have hYd : MDiffAt (T% (cov.along Y' Z')) x :=
    (cov.contMDiff_along (n := 1) hY' hZ' x).mdifferentiableAt (by norm_num)
  have hXd : MDiffAt (T% (cov.along X' Z')) x :=
    (cov.contMDiff_along (n := 1) hX' hZ' x).mdifferentiableAt (by norm_num)
  have hYd0 : MDiffAt (T% (cov.along Y Z)) x := by
    apply hYd.congr_of_eventuallyEq
    filter_upwards [heY] with y hy
    exact congrArg (TotalSpace.mk' E y) hy
  have hXd0 : MDiffAt (T% (cov.along X Z)) x := by
  ...
  have hcZ := IsCovariantDerivativeOn.congr_of_eventuallyEq
    ...
    (by simp) hZeq
  simp only [CovariantDerivative.curvatureAux_apply, CovariantDerivative.along_apply,
